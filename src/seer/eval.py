...
from .data import ComforStream, FolderDataset, load_sample_image
...
def _build_eval_dataset(dataset, real_dirs=None, fake_dirs=None):
    ...
    if spec is None:
        raise ValueError(f"Unknown eval dataset '{dataset}' (try {list(EVAL_SPECS)})")
...
def main(argv=None):
    ...
    p.add_argument("--dataset", default="comfor_eval",
                   help=f"one of {list(EVAL_SPECS)} or 'folders'")
